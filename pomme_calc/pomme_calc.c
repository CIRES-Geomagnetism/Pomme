...
//   control[POS_EM_IND] =    0; 
   ...
   pomme_geodetic(control, fday, in_vals.elev, in_vals.min_lat, in_vals.min_lon, E_st, I_st, IMF_By, f107, Em, &x, &y, &z, &h, &f, &incl, &decl);
	...
	pomme_geodetic(control, fday, in_vals.elev, in_vals.min_lat, in_vals.min_lon, E_st, I_st, IMF_By, f107, Em, &x_t, &y_t, &z_t, &h_t, &f_t, &incl_t, &decl_t);
   ...
   results->magc[0][0][0].inclination = incl_t - incl_m;
   results->magc[0][0][0].horiz_intensity = h_t - h_m;
   results->magc[0][0][0].total_intensity = f_t - f_m;
